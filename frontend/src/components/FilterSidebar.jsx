...
const FilterSidebar = ({ onFilterChange, products, activeFilters }) => {
  ...
  return (
    <div className="filter-container">
      {Object.keys(activeFilters).length > 0 ? <button className="filter-clean-btn" onClick={handleClearFilters}>Temizle</button> : ""}
          ...
          (filterCategory) =>
            activeFilters[filterCategory].length > 0 && (
              <div key={filterCategory}>
                ...
                <p>{filterCategory ==="Fiyat" ? activeFilters[filterCategory].join(" TL - ")+" TL" : activeFilters[filterCategory].join(", ")}</p>
              </div>
            )
        )}
      </div>
